...
libimagequant works with any PNG library. This example uses lodepng, because
it's easier to use than libpng. To run this example, run these commands in the
library directory: make example
...
int main(int argc, char* argv[]) {
  ...
  unsigned int status = lodepng_decode32_file(&raw_rgba_pixels, &width, &height,
                                              input_png_file_path);
  if (status) {
  ...
  liq_image* input_image =
      liq_image_create_rgba(handle, raw_rgba_pixels, width, height, 0);
  ...
  unsigned int out_status =
      lodepng_encode(&output_file_data, &output_file_size, raw_8bit_pixels,
                     width, height, &state);
  if (out_status) {
  ...
  liq_result_destroy(quantization_result);  // Must be freed only after you're
